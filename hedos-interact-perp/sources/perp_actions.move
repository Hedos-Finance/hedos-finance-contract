module hedos::perp_actions {
    ...
    public entry fun init_vault(signer: &signer) acquires PerpVaultRef {
        only_admin(signer);
        ...
        if (!exists<PerpVaultRef>(HEDOS)) {
            ...
            let vault_ref = borrow_global_mut<PerpVaultRef>(HEDOS);
            vault_ref.short_address = new_short_address;
            vault_ref.short_extend_ref = new_short_extend_ref;
            vault_ref.long_address = new_long_address;
            vault_ref.long_extend_ref = new_long_extend_ref;
        };

        emit(CreateNewVault {
            new_vault_address: new_short_address
        });

        emit(CreateNewVault {
            new_vault_address: new_long_address
        });
    }

    public entry fun transfer_all_usdc_back(
        signer: &signer,
        ...
        let short_signer = &object::generate_signer_for_extending(&vault_ref.short_extend_ref);
        let long_signer = &object::generate_signer_for_extending(&vault_ref.long_extend_ref);

        if (get_usdc_balance(short_address) > 0) {
            transfer_usdc(short_signer, get_vault_address(), get_usdc_balance(short_address));

        };
        if (get_usdc_balance(long_address) > 0) {
            transfer_usdc(long_signer, get_vault_address(), get_usdc_balance(long_address));
        };
    }
    ...
    public entry fun perp_open_position(
        ...
        if (protocol == merkle()) {
            if (!is_long) {
                simple_trade_by_leverage(short_signer, short_address, collateral_delta, leverage, is_long, true, market_skew, pair);
            } else {
                transfer_usdc(short_signer, long_address, collateral_delta);
                simple_trade_by_leverage(long_signer, long_address, collateral_delta, leverage, is_long, true, market_skew, pair);
            };
        } else {
            abort 1;
        };
    ...
    public entry fun perp_close_position(
        ...
        if (protocol == merkle()) {
            if (!is_long) {
                simple_trade_by_leverage(short_signer, short_address, collateral_delta, leverage, is_long, false, market_skew, pair);
            } else {
                simple_trade_by_leverage(long_signer, long_address, collateral_delta, leverage, is_long, false, market_skew, pair);
            };
        } else {
            abort 1;
        };
    ...
    public entry fun perp_open_position_by_size(
        ...
        if (protocol == merkle()) {
            if (!is_long) {
                simple_trade_by_size(short_signer, short_address, collateral_delta, size_delta, is_long, true, market_skew, pair);
            } else {
                transfer_usdc(short_signer, long_address, collateral_delta);
                simple_trade_by_size(long_signer, long_address, collateral_delta, size_delta, is_long, true, market_skew, pair);
            };
        } else {
            abort 1;
        };
    ...
    public entry fun perp_close_position_by_size(
        ...
        if (protocol == merkle()) {
            if (!is_long) {
                simple_trade_by_size(short_signer, short_address, collateral_delta, size_delta, is_long, false, market_skew, pair);
            } else {
                simple_trade_by_size(long_signer, long_address, collateral_delta, size_delta, is_long, false, market_skew, pair);
            };
        } else {
            abort 1;
        };
