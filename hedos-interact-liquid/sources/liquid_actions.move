module hedos::liquid_actions {
    ...
    public fun get_total_staked(
        token: String, protocol: String, token_out: String
        ...
        if (protocol == amnis() && token == apt()) {
            ...
            let amount_amAPT =
                ((price_stAPT() as u128) * (amount_stAPT as u128) / (PRECISION as u128)) as u64;
            let amount_amAPT_all = amount_amAPT + amount_balance_amAPT;
            let amount_usdc = get_X_to_Y_out(
                amount_amAPT_all, AMAPT_CHOOSEN, USDC_CHOOSEN
            );
